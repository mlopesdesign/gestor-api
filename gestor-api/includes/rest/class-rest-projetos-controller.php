...
/**
 * Endpoints /projetos/* (CRUD).
 */
final class Rest_Projetos_Controller extends Rest_Controller
{
    public function register_routes(): void
    {
        $instance = $this;

        register_rest_route(self::NAMESPACE, '/projetos', [
            [
        ...
        register_rest_route(self::NAMESPACE, '/projetos/(?P<id>[0-9A-HJKMNP-TV-Z]{26})', [
            [
                'methods' => WP_REST_Server::READABLE,
    ...
    public function list_items(WP_REST_Request $request): WP_REST_Response|\WP_Error
    {
        ...
        return $this->ok([
            'items' => array_map([Projeto::class, 'to_dto'], $items),
            'total' => count($items),
        ]);
    }
    ...
    public function get_item(WP_REST_Request $request): WP_REST_Response|\WP_Error
    {
        ...
        if ($row === null) {
            return Response::error('projeto_nao_encontrado', 'Projeto nao encontrado', 404);
        }
    ...
    private function upsert(WP_REST_Request $request, ?string $forced_id = null): WP_REST_Response|\WP_Error
    {
        ...
        try {
            ...
            return $this->ok(Projeto::to_dto($row), $forced_id === null ? 201 : 200);
        } catch (\Throwable $e) {
