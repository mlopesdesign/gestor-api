 ...
 */
final class Rest_Areas_Controller extends Rest_Controller
{
    public function register_routes(): void
    {
        ...
        register_rest_route(self::NAMESPACE, '/areas/(?P<id>[0-9A-HJKMNP-TV-Z]{26})', [
            [
                'methods' => WP_REST_Server::READABLE,
    ...
    public function list_items(WP_REST_Request $request): WP_REST_Response|\WP_Error
    {
        ...
        return $this->ok([
            'items' => array_map([Area::class, 'to_dto'], $items),
            'total' => count($items),
        ]);
    }
    ...
    public function get_item($request): WP_REST_Response|\WP_Error
    {
        ...
        $model = new Area();
        $row = $model->find_by_id($id, $sessao['usuario_id']);
        if ($row === null) {
            return Response::error('area_nao_encontrada', 'Area nao encontrada', 404);
        }
        return $this->ok(Area::to_dto($row));
    }
    ...
    private function upsert(WP_REST_Request $request, ?string $forced_id = null): WP_REST_Response|\WP_Error
    {
        ...
        try {
            ...
            return $this->ok(Area::to_dto($row), $forced_id === null ? 201 : 200);
        } catch (\Throwable $e) {
